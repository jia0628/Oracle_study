...
/*
hm01: connect, resource, dba 롤 권한을 system으로 부여받음. 

[ 계정 생성 및 권한 부여 단계 ]
예제) user01 계정을 생성하고, 비밀번호는 1234로 부여하시오.
hm01 계정에서 emp 테이블을 조회하고, 데이터를 추가하는 권하는 user01에게 부여하시오.

1. 계정명에서 c##을 제거하도록 설정 (DBA만 가능)
alter session set "_ORACLE_SCRIPT" = true;
2. 계정 생성
create user user01 identified by 1234;
3. 권한 부여
grant select, insert on emp to user01;
4. 테이블 스테이스 사용 설정 (DBA만 가능)
alter user user01 default tablespace users quota unlimited on users;

[ 부여 받은 권한을 다른 사용자에게 다시 부여하는 방법 ]
-- hm01 계정에서 
grant select, insert on emp to user01 with grant option;

-- user01 계정에서 다시 user02 계정에게 권한 부여
grant select, insert on hm01.emp to user02;

[ 모든 사용자에게 한번에 권한을 부여하는 방법 ]
grant select on hm01.emp to public;

-- 권한 회수
revoke select on hm01.emp from public;

create table sgrade as select * from salgrade;
...
-- hm01 계정에서
1. 롤을 생성
create role role_select01;
2. 롤에 권한 부여
grant select on emp to role_select01;
grant select on dept to role_select01;
grant select on sgrade to role_select01;
3. 롤권한을 사용자 부여
grant role_select01 to user01;

*/

-- 사용자(계정) 확인
select * from dba_users where username in ('HM01', 'USER01', 'USER02', 'USER03', 'USER04', 'USER05');
