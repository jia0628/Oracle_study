...
JDK 8 + eclipse java 2020.06
JDK 11 + eclipse jee 2020.09
...
문제14) 사원 테이블에서 사원명이 A로 시작하는 사원의 정보를 조회하시오.
select * from employee
where ename like 'A%';


문제17) 사원 테이블에서 사원명에 세번째 글자가 A인 사원의 정보를 조회하시오.
select * from employee
where ename like '__A'; (X) -> 세번째 글자가 A인 3글자가 되버림
where ename like 'A%';
...
< order by 절의 정렬 방법 >
문제23) 사원 테이블에서 연봉이 높은 순으로 정렬하여 출력하시오.
...
- field(필드) 8개 -> 세로방향, 자바로 치면 클래스안에 멤버변수
-  recode(레코드) -> 가로방향, 데이터
- degree(디그리, 차수) -> 데이터의 규모를 파악함 (필드가 몇개인가)
- cardinality(카디널리티, 기수) -> 데이터가 몇개인지


- varchar2 -> v가 variable(값)



null -> 값을 넣지 않아도 된다
not null -> 반드시 값을 넣어야 한다



정규화한다 -> 하나의 테이블을 기능별로 쪼갠다, 나눈다 => 테이블을 정규화한다
